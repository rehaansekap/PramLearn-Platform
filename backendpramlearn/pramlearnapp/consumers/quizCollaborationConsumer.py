...
class QuizCollaborationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        try:
            ...
            if self.is_connected:  # Double check connection is still active
                ...
                logger.info(f"✅ Notified group members about {user.username} joining")
        except Exception as e:
    ...
    def authenticate_user(self, token):
        ...
        try:
            ...
            logger.info(f"🔑 JWT token decoded successfully for user: {user.username}")
            return user
    ...
    async def disconnect(self, close_code):
        ...
        logger.info(f"📪 Quiz collaboration disconnected: close_code={close_code}")
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            if message_type == "ping":
                ...
                try:
                    ...
                    current_answers = await asyncio.wait_for(
                        self.get_current_answers(), timeout=5.0
                    )

                    logger.info(f"✅ Got current answers: {len(current_answers)} items")
                    ...
                    if self.is_connected:
                        ...
                        logger.info("✅ Sent current_state response successfully")
                ...
                if self.is_connected:
                    success = await self.save_group_answer(
                        question_id, selected_choice, user.id
                    )
            ...
            if self.is_connected:
                await self.send(
                    text_data=json.dumps(
                        {"type": "error", "message": "Format pesan tidak valid"}
                    )
                )
        except Exception as e:
    ...
    async def handle_answer_selection(self, data):
        ...
        if success:
            ...
            await self.send(
                text_data=json.dumps(
                    {"type": "error", "message": "Failed to save answer"}
                )
            )
    ...
    async def broadcast_ranking_update(self, quiz_id, material_id):
        ...
        try:
            ...
            logger.info(f"📡 Sending ranking update to group: {ranking_group_name}")
    ...
    def is_group_member(self):
        ...
        try:
            return GroupMember.objects.filter(
                group_id=self.group_id, student=self.scope["user"]
            ).exists()
        except Exception as e:
        ...
        try:
            ...
            try:
                group_quiz = GroupQuiz.objects.get(
                    quiz_id=self.quiz_id, group_id=self.group_id
                )
    ...
    def save_group_answer(self, question_id, selected_choice, user_id):
        try:
            ...
            group_quiz = GroupQuiz.objects.get(quiz=quiz, group_id=self.group_id)
    ...
    async def send_current_state(self):
        ...
        try:
            ...
            await self.send(
                text_data=json.dumps(
                    {"type": "current_state", "answers": current_answers}
                )
            )
        except Exception as e:
