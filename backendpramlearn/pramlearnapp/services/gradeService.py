...
from ..models import Grade, GradeStatistics, Achievement, Assignment, Quiz, GroupQuiz, GroupMember
import logging
...
class GradeService:
    ...
    def get_subject_breakdown(self):
        ...
        for subject, data in subjects.items():
            breakdown[subject] = {
                'average': round(sum(data['total_grades']) / len(data['total_grades']), 1),
                'quiz_average': round(sum(data['quiz_grades']) / len(data['quiz_grades']), 1) if data['quiz_grades'] else 0,
                'assignment_average': round(sum(data['assignment_grades']) / len(data['assignment_grades']), 1) if data['assignment_grades'] else 0,
    ...
    def check_and_award_achievements(self):
        ...
        quiz_avg = grades.filter(type='quiz').aggregate(
            avg=Avg('grade'))['avg']
        ...
        assignment_avg = grades.filter(
            type='assignment').aggregate(avg=Avg('grade'))['avg']
        ...
        return [{'type': a.type, 'title': a.title, 'description': a.description, 'icon': a.icon} for a in new_achievements]
...
def create_grade_from_submission(submission):
    ...
    try:
        ...
        # ✅ PERBAIKAN: Pastikan submission sudah final (bukan draft)
        if submission.is_draft:
            logger.warning(f"❌ Submission {submission.id} is still a draft!")
            return None

        # ✅ PERBAIKAN: Pastikan assignment submission sudah ada grade
        if submission.grade is None:
            logger.warning(
                f"❌ Submission {submission.id} has no grade calculated!")
            return None
        ...
        # ✅ PERBAIKAN: Create grade record dengan validation yang lebih ketat
        try:
...
def create_grade_from_quiz_attempt(quiz_attempt):
    ...
    try:
        ...
        correct_answers = quiz_attempt.answers.filter(
            is_correct=True
        ).count()
...
def create_grade_from_group_quiz(group_quiz, student):
    ...
    try:
        ...
        # ✅ PERBAIKAN: Cek apakah GroupQuiz sudah disubmit
        ...
        existing_grade = Grade.objects.filter(
            ...
            Q(title__icontains='Group') | Q(title=quiz.title)
        ).first()
        ...
        # ✅ PERBAIKAN: Get the group quiz result dengan error handling yang lebih baik
        try:
        ...
        try:
            ...
            grade = Grade.objects.create(
                ...
                subject_name=quiz.material.subject.name if quiz.material and quiz.material.subject else 'Unknown',
            ...
            try:
                ...
                grade_stats.update_statistics()
                logger.info(
                    f"✅ Statistics updated for student {student.username}")
            except Exception as stats_error:
                logger.error(f"❌ Error updating statistics: {stats_error}")
