                ...
                return Response(
                    {
                        ...
                        "score": quiz_score,
                        "completed_at": (
                            quiz_result.completed_at
                            if "quiz_result" in locals()
                            else None
                        ),
                        "group": {
...
class SubmitGroupQuizView(APIView):
    ...
    def post(self, request, quiz_slug):
        try:
            ...
            # Get user's group
            user_group = GroupMember.objects.filter(
                student=user, group__material=quiz.material
            ).first()
            ...
            if not group_quiz:
                ...
                return Response(
                    {"error": "Group quiz not found"}, status=status.HTTP_404_NOT_FOUND
                )

            # Check if already submitted
            if group_quiz.submitted_at is not None:
                logger.warning(f"⚠️ Quiz already submitted at {group_quiz.submitted_at}")
                return Response(
            ...
            with transaction.atomic():
                ...
                if failed_grades:
                    logger.warning(f"⚠️ Some grades failed to create: {failed_grades}")
...
class GroupQuizResultsView(APIView):
    ...
    def get(self, request, quiz_slug):
        try:
            logger.info(f"🔍 DEBUG: Fetching group quiz results for slug: {quiz_slug}")
            ...
            logger.info(f"📤 Final response data keys: {list(results_data.keys())}")
            logger.info(
