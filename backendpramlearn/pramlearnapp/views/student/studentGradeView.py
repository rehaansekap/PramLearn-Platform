        ...
        try:
            ...
            try:
                ...
                if grade.title and "group" in grade.title.lower():
                    ...
                    quiz_title_clean = grade.title.replace('Quiz ', '').replace('(Group)', '').strip()
                    ...
                    if possible_quiz:
                        ...
                        if user_group_member:
                            ...
                            logger.info(f"Found group data: {group_data['name']} with {len(group_data['members'])} members")
                        else:
                            logger.warning("User not found in any group for this quiz material")
                else:
                    # Individual quiz
                    is_group_quiz = False
                    ...
                    if possible_quiz:
                        quiz = possible_quiz
                        logger.info(f"Found matching individual quiz: {quiz.title}")
            ...
            if quiz and is_group_quiz:
                    ...
                    if user_group_member:
                        ...
                        if group_quiz:
                            ...
                            for question in all_questions:
                                ...
                                submission = group_submissions.filter(question=question).first()
                            ...
                            logger.info(f"Found {len(questions_data)} questions for group quiz")
                        else:
                            logger.warning("GroupQuiz not found for user's group")
                    else:
                        logger.warning("User not in any group for this quiz material")
                ...
                try:
                    ...
                    if quiz_attempt:
                        ...
                        logger.info(f"Found {len(questions_data)} questions for individual quiz")
                    else:
                        logger.warning("No StudentQuizAttempt found for this quiz and user")
                        
                except Exception as e:
                    logger.error(f"Error getting individual quiz questions: {e}")
            ...
            if quiz:
                if hasattr(quiz, 'duration'):
                    ...
                    duration = int((quiz.end_time - quiz.start_time).total_seconds() // 60)
            ...
            logger.info(f"Quiz review for ID {attempt_id}: {correct_count}/{len(questions_data)} correct, is_group_quiz: {is_group_quiz}")
            return Response(response_data)
...
class GroupQuizReviewView(APIView):
    ...
    def get(self, request, attempt_id):
        """Get group quiz review details"""
        download_format = request.GET.get('format')

        if download_format:
            return self.download_group_quiz_report(request, attempt_id, download_format)

        try:
            from pramlearnapp.models.group import GroupQuiz, GroupQuizSubmission, GroupMember
            from pramlearnapp.models.quiz import Quiz, Question
            from pramlearnapp.models import Grade

            # 1. Cari Grade (pastikan type='quiz')
            grade = get_object_or_404(
                Grade,
                id=attempt_id,
                student=request.user,
            ...
            # 2. Cari quiz yang sesuai
            quiz = Quiz.objects.filter(title__icontains=grade.title).first()
            if not quiz:
                return Response({"error": "Quiz tidak ditemukan"}, status=404)
            ...
            if not user_group_member:
                return Response({"error": "Anda tidak terdaftar dalam kelompok untuk quiz ini"}, status=404)

            # 4. Cari GroupQuiz untuk group dan quiz ini
            group_quiz = GroupQuiz.objects.filter(
                quiz=quiz,
                group=user_group_member.group
            ).first()
            if not group_quiz:
                return Response({"error": "GroupQuiz tidak ditemukan"}, status=404)

            # 5. Ambil semua soal
            questions = quiz.questions.all()
            # 6. Ambil semua jawaban group
            submissions = GroupQuizSubmission.objects.filter(
                group_quiz=group_quiz
            ).select_related('question', 'student')

            # 7. Build questions data
            questions_data = []
            correct_count = 0
            incorrect_count = 0
            unanswered_count = 0
            ...
            for question in questions:
                ...
                if submission:
                    ...
                    # User answer
                    user_answer = submission.selected_choice
                    # Correct answer
            ...
            # 8. Build group_data
            group_members = GroupMember.objects.filter(
                group=user_group_member.group).select_related('student')
            group_data = {
                'id': user_group_member.group.id,
                'name': user_group_member.group.name,
                'code': user_group_member.group.code,
            ...
            response_data = {
                ...
                'summary': {
                    ...
                    'incorrect_answers': incorrect_count,
                    'unanswered': unanswered_count,
                    'accuracy_percentage': (correct_count / questions.count() * 100) if questions.count() else 0
                },
                'is_group_quiz': True,
                'group_data': group_data
            }
    ...
    def download_group_quiz_report(self, request, attempt_id, format_type='pdf'):
        """Download group quiz report - mirip individual tapi disesuaikan untuk group"""
        try:
            ...
            if format_type == 'pdf':
                return self.generate_quiz_pdf_report(attempt, request.user)
            else:
                return self.generate_quiz_text_report(attempt, request.user)
    ...
    def generate_quiz_pdf_report(self, attempt, user):
        """Generate PDF report untuk quiz mirip dengan assignment feedback"""
        try:
            from reportlab.lib.enums import TA_CENTER, TA_LEFT
            buffer = BytesIO()
            doc = SimpleDocTemplate(
            ...
            # Header - mirip assignment feedback
            header_style = ParagraphStyle(
                "Header",
                parent=styles["Heading1"],
                fontSize=18,
                alignment=TA_CENTER,
                textColor=colors.HexColor("#3a3f5c"),
                spaceAfter=12,
            )
            story.append(Paragraph("Quiz Results Report", header_style))
            story.append(Spacer(1, 8))

            # Quiz title & subject
            quiz = attempt.quiz
            story.append(Paragraph(
                f'<b>{quiz.title}</b> - <font color="#888">{quiz.material.subject.name if quiz.material and quiz.material.subject else "Mata Pelajaran"}</font>',
                styles["Normal"]
            ))
            story.append(Spacer(1, 8))

            # Status badge - mirip assignment
            nilai = float(attempt.score or 0)
            if nilai >= 85:
                badge = "🌟 Sangat Baik"
                badge_color = "#52c41a"
            elif nilai >= 70:
            ...
            badge_table = Table(
                [[Paragraph(
                    f'<font color="{badge_color}"><b>{badge}</b></font>', styles["Normal"])]],
                style=[
                    ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#f6ffed") if nilai >=
                     85 else colors.HexColor("#fffbe6") if nilai >= 70 else colors.HexColor("#fff1f0")),
                    ("BOX", (0, 0), (-1, -1), 1, colors.HexColor(badge_color)),
                    ("ALIGN", (0, 0), (-1, -1), "CENTER"),
                    ("LEFTPADDING", (0, 0), (-1, -1), 8),
                    ("RIGHTPADDING", (0, 0), (-1, -1), 8),
                ]
            ...
            # Quiz feedback
            feedback_text = getattr(
                attempt, 'teacher_feedback', None) or "Quiz telah diselesaikan dengan baik."
            feedback_color = "#e6f7ff"
            feedback_para = Paragraph(
                f'📝 <b>Feedback Quiz:</b><br/><font color="#888"><i>{feedback_text}</i></font>',
                styles["Normal"]
            )
            feedback_table = Table(
                [[feedback_para]],
                style=[
                    ("BACKGROUND", (0, 0), (-1, -1),
                     colors.HexColor(feedback_color)),
                    ("BOX", (0, 0), (-1, -1), 1, colors.HexColor("#91d5ff")),
                    ("LEFTPADDING", (0, 0), (-1, -1), 8),
                    ("RIGHTPADDING", (0, 0), (-1, -1), 8),
                ]
            )
            ...
            # Detail Penilaian Quiz (2 kolom) - mirip assignment
            info_style = ParagraphStyle(
                "info", parent=styles["Normal"], fontSize=11, spaceAfter=4)

            # Kiri: Informasi Penilaian
            info_penilaian = [
                [Paragraph('<b>📊 Informasi Penilaian</b>', styles["BodyText"])],
                [Paragraph(
                    f'Skor Akhir: <font color="#52c41a"><b>{nilai:.1f}/100</b></font>', info_style)],
                [Paragraph(
                    f'Grade: <b>{("A" if nilai >= 90 else "B" if nilai >= 80 else "C" if nilai >= 70 else "D" if nilai >= 60 else "E")}</b>', info_style)],
                [Paragraph(
            ...
            # Kanan: Timeline Quiz
            tgl_selesai = attempt.completed_at.strftime(
                "%d %B %Y") if attempt.completed_at else "-"
            durasi = getattr(attempt, 'duration_minutes', quiz.duration)
            timeline = [
                [Paragraph('<b>📅 Timeline Quiz</b>', styles["BodyText"])],
                [Paragraph(f'Diselesaikan: <b>{tgl_selesai}</b>', info_style)],
                [Paragraph(f'Durasi: <b>{durasi} menit</b>', info_style)],
                [Paragraph('Tipe: <b>Quiz Individual</b>', info_style)],
            ]
            ...
            # Review Jawaban
            quiz_answers = StudentQuizAnswer.objects.filter(
                quiz_attempt=attempt).select_related('question')
            if quiz_answers.exists():
                story.append(
                    Paragraph('<b>Review Jawaban</b>', styles["BodyText"]))
                ...
                for idx, answer in enumerate(quiz_answers, 1):
                    ...
                    # Correct answer
                    correct_answer = "N/A"
                    if question.question_type == 'multiple_choice' and hasattr(question, 'correct_choice'):
                        choice_map = {
                            'A': question.choice_a, 'B': question.choice_b,
                            'C': question.choice_c, 'D': question.choice_d
                        }
                    ...
                    # Box untuk setiap soal
                    soal_table = Table(
                        [
                            [Paragraph(
                                f"<b>Soal {idx}:</b> {question.text}", info_style)],
                            [Paragraph(
                                f"<b>Jawaban Anda:</b> {user_answer}", info_style)],
                            [Paragraph(
                                f"<b>Jawaban Benar:</b> {correct_answer}", info_style)],
                            [Paragraph(
                                f"<b>Status:</b> {'Benar ✓' if is_correct else 'Salah ✗'}", info_style)],
                        ],
                        style=[
                            ("BOX", (0, 0), (-1, -1), 1,
                             colors.HexColor("#52c41a" if is_correct else "#ff4d4f")),
                            ("BACKGROUND", (0, 0), (-1, -1),
                             colors.HexColor("#f6ffed" if is_correct else "#fff2f0")),
                            ("LEFTPADDING", (0, 0), (-1, -1), 8),
                            ("RIGHTPADDING", (0, 0), (-1, -1), 8),
                            ("TOPPADDING", (0, 0), (-1, -1), 4),
                            ("BOTTOMPADDING", (0, 0), (-1, -1), 4),
                ...
                summary_para = Paragraph(
                    f'<b>Ringkasan:</b> {correct_count} benar dari {total_questions} soal ({(correct_count/total_questions*100):.1f}%)',
                    info_style
                )
                story.append(summary_para)
            else:
                story.append(Paragraph(
                    '<font color="#888">Detail jawaban tidak tersedia untuk quiz ini.</font>', info_style))

            # Saran Pengembangan - mirip assignment
            story.append(Spacer(1, 16))
            if nilai >= 90:
                saran = ["Selamat! Hasil quiz sangat memuaskan.",
                         "Pertahankan prestasi belajar yang baik.", "Bantu teman-teman yang membutuhkan bantuan."]
                saran_color = "#f6ffed"
            elif nilai >= 75:
                saran = ["Hasil quiz cukup baik.", "Review kembali materi yang belum dikuasai.",
                         "Latihan soal tambahan untuk memperkuat pemahaman."]
                saran_color = "#e6f7ff"
            else:
                saran = ["Perlu perbaikan untuk quiz selanjutnya.", "Pelajari kembali materi dasar.",
                         "Konsultasi dengan guru untuk penjelasan lebih detail."]
                saran_color = "#fffbe6"

            saran_table = Table(
                [[Paragraph(
                    "<br/>".join([f"💡 <b>Saran Pengembangan:</b>"] + saran), info_style)]],
                style=[
                    ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor(saran_color)),
            ...
            # Footer
            footer_style = ParagraphStyle(
                'Footer', parent=styles['Normal'], fontSize=9, textColor=colors.gray, alignment=TA_CENTER)
            story.append(Spacer(1, 20))
            story.append(
                Paragraph("Generated by PramLearn System", footer_style))
            story.append(Paragraph(
            ...
            response = HttpResponse(content_type='application/pdf')
            response['Content-Disposition'] = f'attachment; filename="quiz-report-{attempt.id}.pdf"'
            response.write(pdf_data)
            return response

        except Exception as e:
            logger.error(f"Error generating quiz PDF: {e}")
            return self.generate_quiz_text_report(attempt, user)

    def generate_quiz_text_report(self, attempt, user):
        """Generate simple text report sebagai fallback"""
        try:
            quiz = attempt.quiz
            report_content = f"""Quiz Results Report
==================

Student: {user.get_full_name() or user.username}
Quiz: {quiz.title}
Subject: {quiz.material.subject.name if quiz.material and quiz.material.subject else 'N/A'}
Score: {attempt.score}/100
Completed: {attempt.completed_at.strftime('%d %B %Y, %H:%M') if attempt.completed_at else 'N/A'}

Generated by PramLearn System
Generated on {datetime.datetime.now().strftime('%d %B %Y at %H:%M')}
"""

            response = HttpResponse(report_content.encode(
                'utf-8'), content_type='text/plain')
            response['Content-Disposition'] = f'attachment; filename="quiz-report-{attempt.id}.txt"'
            return response
