...
class StudentMaterialProgressView(APIView):
    ...
    def get(self, request, material_id):
        ...
        try:
            ...
            total_completion = self.calculate_total_completion(request.user, material)
    ...
    def put(self, request, material_id):
        ...
        try:
            ...
            real_completion = self.calculate_total_completion(request.user, material)

            # Update progress hanya jika ada perubahan yang valid
            if "completion_percentage" in request.data:
                requested_percentage = float(request.data.get("completion_percentage"))
            ...
            if "time_spent" in request.data:
                progress.time_spent = max(0, int(request.data.get("time_spent")))
    ...
    def calculate_total_completion(self, user, material):
        ...
        total_videos = (
            material.youtube_videos.filter(url__isnull=False).exclude(url="").count()
        )
        ...
        pdf_activities = StudentMaterialActivity.objects.filter(
            student=user, material=material, activity_type="pdf_opened"
        ).count()
        ...
        video_activities = StudentMaterialActivity.objects.filter(
            student=user, material=material, activity_type="video_played"
        ).count()
        ...
        for quiz in material.quizzes.all():
            if quiz.is_group_quiz:
                ...
                user_group = GroupMember.objects.filter(
                    student=user, group__material=material
                ).first()
...
class StudentMaterialBookmarkView(APIView):
    ...
    def get(self, request, material_id):
        ...
        try:
            ...
            serializer = StudentMaterialBookmarkSerializer(bookmarks, many=True)
            return Response(serializer.data)
...
class StudentMaterialActivityView(APIView):
    ...
    def post(self, request, material_id):
        try:
            ...
            if real_completion >= 100 and activity_type in [
                "pdf_opened",
                "video_played",
            ]:
                logger.info(
            ...
            if activity_type == "quiz_completed" and quiz_id:
                ...
                if existing_activity:
                    ...
                    return Response(
                        {"detail": "Quiz activity already recorded."}, status=409
                    )
                ...
                logger.info(f"✅ Quiz completion activity recorded: quiz_{quiz_id}")
            ...
            if activity_type == "assignment_submitted" and assignment_id:
                ...
                if existing_activity:
                    ...
                    return Response(
                        {"detail": "Assignment activity already recorded."}, status=409
                    )

                # Validasi apakah assignment benar-benar sudah disubmit
                submission = AssignmentSubmission.objects.filter(
                    student=request.user, assignment_id=assignment_id, is_draft=False
                ).first()

                if not submission:
                    return Response(
                        {"detail": "Assignment not submitted yet."}, status=400
                    )
                ...
                return Response(
                    {"detail": "Assignment submission recorded."}, status=201
                )

            if (
                activity_type in ["pdf_opened", "video_played"]
                ...
                if existing_activity:
                    ...
                    return Response(
                        {"detail": f"{activity_type} already recorded."}, status=409
                    )
            ...
            return Response(
                {"detail": "Invalid activity type or missing parameters."}, status=400
            )
