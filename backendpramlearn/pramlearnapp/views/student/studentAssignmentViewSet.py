    ...
    def get(self, request):
        ...
        try:
            ...
            if not class_students.exists():
                return Response(
                    {"message": "No classes found for this student", "assignments": []}
                )
            ...
            if not subject_ids:
                return Response(
                    {"message": "No subjects found for this student", "assignments": []}
                )
            ...
            assignments = (
                Assignment.objects.filter(material_id__in=material_ids)
            ...
            for assignment in assignments:
                ...
                subject_name = (
                    ...
                    if assignment.material.subject
                    else "Unknown"
                )
                ...
                assignment_dict = {
                    ...
                    "submitted_at": (
                        submission.submission_date.isoformat() if submission else None
                    ),
...
class StudentAssignmentQuestionsView(APIView):
    ...
    def get(self, request, assignment_id):
        ...
        try:
            ...
            user = request.user
            assignment = Assignment.objects.select_related("material__subject").get(
                id=assignment_id
            )
            ...
            subject_classes = SubjectClass.objects.filter(
                class_id__in=class_ids, subject=assignment.material.subject
            )
...
class StudentAssignmentDraftView(APIView):
    ...
    def get(self, request, assignment_id):
        ...
        try:
            draft = StudentAssignmentDraft.objects.get(
                student=request.user, assignment_id=assignment_id
            )
    ...
    def put(self, request, assignment_id):
        ...
        try:
            ...
            return Response(
                {"status": "Draft saved successfully", "last_saved": draft.last_saved}
            )
    ...
    def post(self, request, assignment_id):
        ...
        try:
            ...
            if existing_submission:
                logger.info(f"Submission is_draft: {existing_submission.is_draft}")
            ...
            with transaction.atomic():
                ...
                if not existing_submission:
                    ...
                    logger.info(f"✅ New final submission created: {submission.id}")
                else:
                    ...
                    submission.save()
                    logger.info(f"✅ Draft submission finalized: {submission.id}")

                # Calculate assignment score first
                submission.calculate_and_save_score()
                logger.info(f"✅ Assignment score calculated: {submission.grade}")
            ...
            logger.info(f"   - Grade Record Created: {'Yes' if grade else 'No'}")
    ...
    def get(self, request, assignment_id):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to log student activity: {str(activity_error)}")
...
class StudentAssignmentBySlugView(APIView):
    ...
    def get(self, request, assignment_slug):
        ...
        try:
            ...
            assignment = (
                Assignment.objects.select_related("material__subject")
                .prefetch_related("questions")
                .get(slug=assignment_slug)
            )
            ...
            return Response(
                {"assignment": assignment_data, "submissions": submissions_data},
