...
from pramlearnapp.models import (
    Material, Assignment, AssignmentQuestion, AssignmentSubmission,
    AssignmentAnswer, ClassStudent, SubjectClass
)
    ...
    def get(self, request, material_slug):
        ...
        try:
            ...
            for assignment in assignments:
                ...
                submissions = assignment.assignmentsubmission_set.filter(
                    is_draft=False)
                ...
                is_overdue = timezone.now() > assignment.due_date if assignment.due_date else False
                ...
                assignment_dict = {
                    ...
                    'status': self.get_assignment_status(assignment, total_submissions, total_students)
                }
            ...
            total_pending = sum(a['pending_submissions']
                                for a in assignments_data)
            overall_avg_grade = sum(a['average_grade'] for a in assignments_data) / \
                total_assignments if total_assignments > 0 else 0
            overall_submission_rate = sum(
                a['submission_rate'] for a in assignments_data) / total_assignments if total_assignments > 0 else 0
            ...
            response_data = {
                ...
                'statistics': {
                    ...
                    'assignments_overdue': sum(1 for a in assignments_data if a['is_overdue'])
                }
            }
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                if questions_data:
                    ...
                    existing_questions = {
                        q.id: q for q in assignment.questions.all()}
                    ...
                    for question_data in questions_data:
                        ...
                        if question_id and question_id in existing_questions:
                            ...
                            # PERBAIKAN: pastikan explanation disimpan
        ...
        try:
            ...
            for submission in submissions:
                ...
                submission_dict = {
                    ...
                    'is_late': submission.submission_date > assignment.due_date if assignment.due_date else False
                }
            ...
            response_data = {
                'assignment': {
                    ...
                    'due_date': assignment.due_date.isoformat() if assignment.due_date else None,
                    'created_at': assignment.created_at.isoformat(),
                    'questions': questions_data  # PERBAIKAN: pastikan questions dikirim dengan benar
                },
                'submissions': submissions_data,
                'statistics': {
                    'total_submissions': len(submissions_data),
                    'graded_count': len([s for s in submissions_data if s['grade'] is not None]),
                    'pending_count': len([s for s in submissions_data if s['grade'] is None]),
                    'average_grade': sum(s['grade'] for s in submissions_data if s['grade'] is not None) / len([s for s in submissions_data if s['grade'] is not None]) if any(s['grade'] is not None for s in submissions_data) else 0,
                    'late_submissions': len([s for s in submissions_data if s['is_late']])
                }
            }
    ...
    def get(self, request, material_slug):
        ...
        try:
            ...
            analytics_data = {
                ...
                'student_performance': self.get_student_performance(assignments, subject_class.class_id),
