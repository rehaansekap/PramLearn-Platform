...
from pramlearnapp.models import Material, Group, GroupMember, CustomUser, SubjectClass, StudentMotivationProfile
...
class TeacherSessionAutoGroupFormationView(APIView):
    ...
    def post(self, request, material_slug):
        ...
        try:
            ...
            if mode == 'homogen':
                groups = self.create_homogeneous_groups(
            ...
            if mode == 'homogen':
                ...
                if uniformity_score > 0.9 and avg_heterogeneity > 0.7:
                    ...
                    quality_message = "✅ Kelompok heterogen dengan distribusi cukup seragam"
                elif avg_heterogeneity > 0.7:
                    quality_message = "✅ Kelompok heterogen terbentuk dengan sangat baik"
            ...
            return Response({
                ...
                'quality_analysis': {
                    ...
                    'interpretation': {
                        'balance': "Sangat Seimbang" if quality_analysis['balance_score'] > 0.8
                        else "Cukup Seimbang" if quality_analysis['balance_score'] > 0.5
                        else "Kurang Seimbang",
                        'heterogeneity': "Sangat Beragam" if quality_analysis['heterogeneity_score'] > 0.7
                        else "Cukup Beragam" if quality_analysis['heterogeneity_score'] > 0.4
                        else "Cukup Homogen" if quality_analysis['heterogeneity_score'] > 0.2
                        else "Sangat Homogen",
                        'uniformity': "Sangat Seragam" if quality_analysis.get('uniformity_score', 0) > 0.8
                        else "Cukup Seragam" if quality_analysis.get('uniformity_score', 0) > 0.6
                        else "Kurang Seragam"
                    }
                }
    ...
    def create_heterogeneous_groups_deap(self, student_data, n_clusters):
        ...
        analyzed_students = [
            s for s in student_data if s['motivation_score'] > 0]
        ...
        motivation_groups = {
            ...
            'Medium': [s for s in analyzed_students if s['motivation_level'] == 'Medium'],
        ...
        def crossover_assignment(ind1, ind2):
            ...
            # Repair if needed
            self.repair_individual(
                ind1, n_groups, analyzed_students, target_sizes)
            self.repair_individual(
                ind2, n_groups, analyzed_students, target_sizes)
        ...
        for level in ['High', 'Medium', 'Low']:
            ...
            if len(counts) > 1:
                ...
                if mean_count > 0:
                    ...
                    uniformity_scores.append(1.0 if all(
                        c == 0 for c in counts) else 0.0)
    ...
    def find_best_groups_for_student(self, individual, student, student_idx, n_groups, all_students):
        """Find best groups for a student during mutation"""
        student_level = student['motivation_level']

        # Calculate current group compositions
        group_compositions = [{'High': 0, 'Medium': 0, 'Low': 0}
        ...
        for group_idx in range(n_groups):
            ...
            other_levels_count = sum(
                composition.values()) - current_level_count
    ...
    def create_heuristic_solutions(self, students, n_groups, target_sizes):
        ...
        solution2 = self.create_balanced_solution(
            students, n_groups, target_sizes)
        solutions.append(solution2)

        # Solution 3: Random with repairs
        for _ in range(3):
            solution = [random.randint(0, n_groups - 1)
    ...
    def create_round_robin_solution(self, students, n_groups):
        ...
        for level in ['High', 'Medium', 'Low']:
            for student_idx in by_motivation[level]:
                ...
                min_group = min(range(n_groups),
                                key=lambda x: group_counters[x])
    ...
    def create_balanced_solution(self, students, n_groups, target_sizes):
        ...
        patterns = self.create_target_patterns(
            n_groups, target_sizes, students)
    ...
    def create_target_patterns(self, n_groups, target_sizes, students):
        ...
        for group_idx, size in enumerate(target_sizes):
            if size == 4:
                ...
                if (remaining['High'] >= 1 and
                    remaining['Medium'] >= 2 and
                        remaining['Low'] >= 1):
                    pattern = {'High': 1, 'Medium': 2, 'Low': 1}
                # Fallback patterns that maintain balance
                elif (remaining['High'] >= 1 and
                      remaining['Medium'] >= 1 and
                      remaining['Low'] >= 2):
                    pattern = {'High': 1, 'Medium': 1, 'Low': 2}
                elif (remaining['High'] >= 2 and
                      remaining['Medium'] >= 1 and
                      remaining['Low'] >= 1):
    ...
    def create_heterogeneous_groups_improved(self, student_data, n_clusters):
        ...
        for student in all_remaining:
            ...
            min_size_group_idx = min(
                range(len(groups)), key=lambda x: len(groups[x]))
    ...
    def validate_motivation_profiles(self, students):
        ...
        analyzed_students = 0
        motivation_distribution = {'High': 0,
                                   'Medium': 0, 'Low': 0, 'Unanalyzed': 0}
        ...
        analyzed_levels = {
            k: v for k, v in motivation_distribution.items() if k != 'Unanalyzed' and v > 0}
    ...
    def create_homogeneous_groups(self, student_data, n_clusters):
        ...
        low_students = [
            s for s in analyzed_students if s['motivation_level'] == 'Low']
        ...
        def create_pure_groups(students, level_name):
            ...
            students_per_group = max(
                2, len(students) // min(n_clusters, len(students)))
            level_groups = []
        ...
        while len(all_pure_groups) > n_clusters:
            ...
            sizes = [(i, len(group))
                     for i, group in enumerate(all_pure_groups)]
            ...
            for i in range(len(sizes) - 1):
                ...
                for j in range(i + 1, len(sizes)):
                    ...
                    if (all_pure_groups[idx1] and all_pure_groups[idx2] and
                        all_pure_groups[idx1][0]['motivation_level'] ==
    ...
    def calculate_optimal_groups(self, total_students):
        ...
        for size3_groups in range(min(3, total_students // 3 + 1)):
            for size4_groups in range((total_students - size3_groups * 3) // 4 + 1):
                remaining = total_students - \
                    (size3_groups * 3) - (size4_groups * 4)
                if remaining >= 0 and remaining % 5 == 0:
                    size5_groups = remaining // 5
                    total_groups = size3_groups + size4_groups + size5_groups
                    if 3 <= total_groups <= 12:
                        sizes = [3] * size3_groups + [4] * \
                            size4_groups + [5] * size5_groups
                        variance = self.calculate_size_variance(sizes)
                        configs.append({
                            'n_groups': total_groups,
                            'sizes': sizes,
        ...
        sizes = [base_size + (1 if i < remainder else 0)
    ...
    def analyze_group_quality(self, groups):
        ...
        for i, group in enumerate(groups):
            if len(group) == 0:
                ...
                if total > 0:
                    ...
                    max_diversity = math.log(min(3, total))
                    group_analysis['heterogeneity_index'] = -diversity_sum / \
                        max_diversity if max_diversity > 0 else 0
        ...
        if non_empty_groups:
            heterogeneity_scores = [g['heterogeneity_index']
                                    for g in analysis['group_details'] if g['size'] > 0]
            analysis['heterogeneity_score'] = np.mean(
                heterogeneity_scores) if heterogeneity_scores else 0
        ...
        if len(group_sizes) > 1:
            ...
            ideal_sizes = [
                ideal_group_size + (1 if i < remainder else 0) for i in range(len(group_sizes))]
            ...
            analysis['balance_score'] = max(
                0, min(1, balance_raw - empty_penalty))
        else:
