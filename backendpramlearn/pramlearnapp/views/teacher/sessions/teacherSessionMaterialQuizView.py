...
from pramlearnapp.models import (
    Material, Quiz, Question, Group, GroupMember,
    GroupQuiz, GroupQuizSubmission, GroupQuizResult, Grade
)
...
class TeacherSessionMaterialQuizView(APIView):
    ...
    def get(self, request, material_slug):
        ...
        try:
            ...
            total_completed = sum(q['completed_submissions']
                                  for q in quizzes_data)
            overall_completion_rate = (
                total_completed / total_assigned_groups * 100) if total_assigned_groups > 0 else 0
            ...
            response_data = {
                ...
                'statistics': {
                    ...
                    'average_questions_per_quiz': round(
                        sum(q['question_count']
                            for q in quizzes_data) / total_quizzes, 1
            ...
            logger.error(
                f"Error in TeacherSessionMaterialQuizView.get: {str(e)}")
            return Response(
        ...
        try:
            ...
            if group_ids:
                groups = Group.objects.filter(
                    id__in=group_ids, material=material)
                for group in groups:
    ...
    def patch(self, request, material_slug, quiz_id):
        ...
        try:
            ...
            if is_active is not None:
                ...
                # 🔥 Broadcast to students if quiz becomes inactive
                if not is_active:
    ...
    def get(self, request, material_slug, quiz_id):
        ...
        try:
            ...
            group_quizzes = GroupQuiz.objects.filter(
                quiz=quiz).select_related('group')
            results_data = []

            for group_quiz in group_quizzes:
                group_members = GroupMember.objects.filter(
                    group=group_quiz.group).select_related('student')
                ...
                try:
                    ...
                    result_data.update({
                        ...
                        'correct_answers': group_quiz.submissions.filter(is_correct=True).count()
                    })
                except GroupQuizResult.DoesNotExist:
                    result_data.update({
                        'score': 0,
...
class TeacherSessionQuizRankingView(APIView):
    ...
    def get(self, request, material_slug, quiz_id):
        ...
        try:
            ...
            group_quizzes = GroupQuiz.objects.filter(
                quiz=quiz
            ).select_related('group').prefetch_related('group__groupmember_set__student')

            ranking_data = []

            for group_quiz in group_quizzes:
                group_members = group_quiz.group.groupmember_set.all()
                member_names = [f"{m.student.first_name} {m.student.last_name}".strip()
                                or m.student.username for m in group_members]
                ...
                rank_item = {
                    ...
                    'status': 'completed' if group_quiz.is_completed else
                    'in_progress' if group_quiz.submissions.exists() else 'not_started',
                    'submitted_at': group_quiz.submitted_at,
                    'total_questions': quiz.questions.count(),
                    'correct_answers': group_quiz.submissions.filter(is_correct=True).count()
                }
            ...
            ranking_data.sort(
                key=lambda x: (-x['score'], x['submitted_at'] if x['submitted_at'] else timezone.now()))
